 ...
 * Dynamically redirects users to their dashboard and viewers to their profile/landing page.
 */
const Forbidden = ({ isViewer = false, profileUsername = null }) => {
  const navigate = useNavigate();

  // --- Smart Navigation Handler ---
  const handleGoBack = () => {
    if (isViewer) {
      // If a viewer, go back to their profile (if username available) or fallback to landing page
      if (profileUsername) {
      ...
      // If an authenticated user, always redirect back to dashboard
      navigate("/dashboard");
    }
  };
      ...
      <div className="relative w-full max-w-md rounded-3xl border border-slate-200 bg-white px-8 py-10 shadow-sm">
        ...
        <div className="flex justify-center">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100">
            <ShieldAlert size={30} className="text-[#1E3A8A]" />
          </div>
        </div>
        ...
        <button
          ...
          className="mt-10 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#1E3A8A] px-6 py-3.5 font-semibold text-white transition-all duration-300 hover:scale-[1.01] hover:bg-[#16213E] active:scale-[0.98] cursor-pointer"
        >
          <ArrowLeft size={18} />
          {isViewer ? "Back to Profile" : "Back to Dashboard"}
        </button>
      </div>
    </div>
  );
