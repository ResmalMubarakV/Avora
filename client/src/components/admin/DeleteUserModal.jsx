...
const DeleteUserModal = ({ selectedUser, onClose, onConfirm, deleteLoading, deleteError }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 dark:bg-slate-950/70 backdrop-blur-md p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-md rounded-2xl sm:rounded-3xl border border-red-200/80 dark:border-red-900/50 bg-white dark:bg-slate-900 shadow-[0_20px_50px_rgba(239,68,68,0.12)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-hidden">
                ...
                <div className="p-6 sm:p-7">
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3 text-red-600 dark:text-red-400">
                            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-red-50 dark:bg-red-950/60 border border-red-100 dark:border-red-900/50 shadow-inner">
                                <ShieldAlert size={20} />
                            </div>
                        ...
                        <button
                            ...
                            className="rounded-xl p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-600 dark:hover:text-slate-300 cursor-pointer transition disabled:opacity-50"
                        >
                            <X size={18} />
                        </button>
                    </div>
                    ...
                    <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mb-4 leading-relaxed font-medium">
                        This action cannot be undone. Permanently delete user <span className="font-bold text-slate-900 dark:text-white">"{selectedUser.name || selectedUser.username}"</span> (@{selectedUser.username}) and all associated records?
                    </p>
