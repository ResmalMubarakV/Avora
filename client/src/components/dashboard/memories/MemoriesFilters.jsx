...
// ==========================================
// MEMORIES FILTERS COMPONENT (Strict Single Horizontal Line Layout)
// ==========================================
/**
 * Renders filter chips, year selector dropdown, and sorting dropdown
 * strictly in a single horizontal line across all viewports without horizontal scrolling.
 */
const MemoriesFilters = ({
  ...
  return (
    <div className="flex flex-row items-center justify-between gap-1.5 sm:gap-2.5 bg-white dark:bg-slate-900/90 p-2 sm:p-3.5 rounded-2xl sm:rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-xs transition-colors">
        ...
        {filterOptions.map((item) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-0.5 sm:gap-1 rounded-lg sm:rounded-xl px-2 sm:px-3 py-1 sm:py-1.5 text-[10px] sm:text-xs font-semibold transition-all duration-200 cursor-pointer shrink-0 ${
                isActive
      ...
      {/* Select Controls: Year & Sorting (Strict Single Horizontal Line) */}
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        ...
        <div className="relative inline-block shrink-0">
          ...
          <select
            ...
            className="appearance-none rounded-lg sm:rounded-xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/60 py-1 sm:py-1.5 pl-6 pr-5 text-[10px] sm:text-xs font-semibold text-slate-700 dark:text-slate-200 outline-none transition-all hover:bg-slate-100 dark:hover:bg-slate-800 focus:border-slate-400 dark:focus:border-indigo-500 cursor-pointer"
          >
            <option value="newest" className="dark:bg-slate-900 dark:text-white">Newest</option>
            <option value="oldest" className="dark:bg-slate-900 dark:text-white">Oldest</option>
