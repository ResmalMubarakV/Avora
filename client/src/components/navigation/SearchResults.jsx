    ...
    if (loading) {
        return (
            <div className="absolute left-0 right-0 top-full mt-3 z-50 rounded-2xl border border-slate-200 bg-white p-8 shadow-2xl">
                <p className="text-center text-sm font-medium text-slate-500">Searching...</p>
            </div>
        );
    }
    ...
    if (!hasResults) {
        return (
            <div className="absolute left-0 right-0 top-full mt-3 z-50 rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-2xl">
                ...
                <p className="mt-2 text-sm text-slate-500">Try searching for another traveler, memory or place.</p>
            </div>
        );
    }

    return (
        <div className="absolute left-0 right-0 top-full mt-3 z-50 max-h-[480px] overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl">
            {users.length > 0 && (
                <div className="border-b border-slate-100 p-4">
                    <p className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">Travelers</p>
                    <div className="space-y-1">
                        {users.map((user) => (
                            ...
                            >
                                {user.profileImage ? (
                                    <img src={user.profileImage} alt={user.name} className="h-10 w-10 rounded-full object-cover shadow-sm" />
                                ) : (
            ...
            {memories.length > 0 && (
                <div className={`${places.length ? "border-b" : ""} border-slate-100 p-4`}>
                    <p className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">Memories</p>
                    <div className="space-y-1">
                        {memories.map((memory) => (
                            <Link
                                key={memory._id}
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <p className="truncate text-sm font-semibold text-slate-900">{memory.title}</p>
                                    <p className="truncate text-xs text-slate-500">📍 {memory.location}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
